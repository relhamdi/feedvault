...
@router.post("/", response_model=FeedCreateResponse, status_code=201)
def create_feed(feed_in: FeedCreate, session: Session = Depends(get_session)):
    source = get_or_404(session, Source, feed_in.source_id)
    ...
    return FeedCreateResponse(feed=FeedRead.model_validate(feed), warning=warning)
